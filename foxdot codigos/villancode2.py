...
q1 >> play(" o",dur=2,room=1).every(8,"stutter",room=10,mix=10)

q8 >> play("(lt) ",dur=2,room=5)

q6 >> play("(elb) (zpf) ",dur=0.5,sample=[0,1,2,3,4,5])

q3 >> feel([0,4,5],dur=8,amp=5)

q2 >> glass([4,11,12],dur=8, amp=3)

q9 >> pads([(4,11,12),(0,4,5)],dur=8, amp=0.4, chop=58,echo=4,cut=1/4)

q5 >> quin([0,4],dur=[0.25,0.5],amp=0.6,cut=1/4)

q6 >> dirt([0,4,5,7],dur=[0.25],amp=[0.5,0.3,0.5,0.7,1],cut=1/4)

q4 >> growl([0,4,5],dur=8, amp=0.5,oct=3)

q7 >> pasha([0,4,5,7],dur=8, amp=0.2,oct=7,slide=4,slidedelay=3,bend=6)
...
e2 >> sawbass([0,4,3,1],dur=[4,8,2],echotime=4,vib=4,beat_dur=4, amp=0.8)
e3 >> ripple([2,4,2,4],dur=[0.5,2,0.5,1],chop=12,vib=4,beat_dur=4)
e7 >> keys([(0,2,4),(4,6,8),(3,5,7),(1,5,9,12)],dur=[4,8,2]).every(4,"amen")#.every(8,"stutter",2,chop=32)


e4 >> snick([0,2,7,8,12],dur=PDur(3,8),amp=2,echo=0.25)
e5 >> play("Xo", dur = PDur(2,8))#.every(12,"amen").every(8,"stutter",2,chop=32)
e6 >> play("f  U", dur = PDur(5,8),sample=[0,1,2,3,4],amp=0.5)#.every(4,"amen").every(8,"stutter",2,chop=32)
...
a1 >> sinepad([4,5,4,2,8,8,6,6,7,7,4],dur=[12] ,oct=5 , amp = 0.4)
b1 >> bell([4,5,4,2,8,8,6,6,7,7,4],dur=[12] ,oct=5, amp=0.4)
c1 >> play("x n",dur=3, amp=2)

a1 >> sinepad([4,5,4,2,8,8,6,6,7,7,4],dur=[6] ,oct=5 , amp = 0.3)
b1 >> bell([4,5,4,2,8,8,6,6,7,7,4],dur=[6] ,oct=5, amp=0.2)
a2 >> sinepad([2,4,6,6,8,8,4,4,2],dur=[6] ,oct=4, amp = 0.3)
...
a3 >> soprano([0,2,3,3,6,6,2,2,7],dur=[3,3,2,1,2,1,2,1,3] ,oct=2)
b1 >> sinepad([4,5,4,2,8,8,6,6,7,7,4],dur=[1.5,0.5,1,3,2,1,2,1,2,1,3] ,oct=5, amp=0.2)
b2 >> sinepad([2,4,6,6,8,8,4,4,2],dur=[3,3,2,1,2,1,2,1,3] ,oct=3, amp= 0.2)
b3 >> sinepad([0,2,3,3,6,6,2,2,7],dur=[3,3,2,1,2,1,2,1,3] ,oct=1, amp=0.7)
c1 >> play("[oo]",dur=PDur(3,12), amp=0.3).every(4,"amen")
...
b1 >> sinepad([4,5,4,2,8,8,6,6,7,7,4],dur=[1.5,0.5,1,3,2,1,2,1,2,1,3] ,oct=3, amp=0.2)
b2 >> sinepad([2,4,6,6,8,8,4,4,2],dur=[3,3,2,1,2,1,2,1,3] ,oct=5, amp= 0.2)
b3 >> sinepad([0,2,3,3,6,6,2,2,7],dur=[3,3,2,1,2,1,2,1,3] ,oct=1, amp=0.3) 
...
c1 >> play("[oo]",dur=PDur(3,9), amp=0.4)
c2 >> play("Xon",dur=PDur(3,9), amp=1)
c3 >> play("#",dur=6, amp=0.3, room=2)
c4 >> play("xn",amp=0.6, sample=[0,PRand(7)]).every(6,"stutter",4,dur=3).every(12, "amen")
...
a2 >> soprano([2,4,6,6,8,8,4,4,2],dur=[3,3,2,1,2,1,2,1,3] ,oct=4)
a3 >> soprano([0,2,3,3,6,6,2,2,7],dur=[3,3,2,1,2,1,2,1,3] ,oct=5)
b1 >> sinepad([4,5,4,2,8,8,6,6,7,7,4],dur=[1.5,0.5,1,3,2,1,2,1,2,1,3] ,oct=1, amp=0.2)
b2 >> sinepad([2,4,6,6,8,8,4,4,2],dur=[3,3,2,1,2,1,2,1,3] ,oct=3, amp= 0.2)
b3 >> sinepad([0,2,3,3,6,6,2,2,7],dur=[3,3,2,1,2,1,2,1,3] ,oct=5, amp=0.3)
...
c4 >> play("xn",amp=1, sample=[0,PRand(7)]).every(6,"stutter",4,dur=3).every(12, "amen")
...
p2 >> pluck([0, 1, 2, 3, 4, 5, 6, 7],dur=PDur(3,8)).penta().every(8,"trim",6)
...
w1 >> keys([(0,4,7,11),(1,5,9,12)],dur=[2,14],amp=0.6, cut=1)
w2 >> sinepad([(0,4,7,11),(1,5,9,12)],dur=[2,14],amp=0.5).solo()

q1 >> play(" o",dur=2,room=1)#.every(8,"stutter",room=10,mix=10)

q8 >> play("V t ",dur=2,room=5)

w3 >> arpy([0,2,5,6,7],dur=[0.25,0.25,0.25,0.25,0.5,14],oct=7).penta()

w4 >> bug([0,1,2,3,4,5],chop=[1,2,4,8],amp=[0.1,0.3,0.5,0.7]).penta()

q6 >> play("(elb) (zpf) ",dur=0.5,sample=[0,1,2,3,4,5])

w5 >> jbass([0,1,5,7],dur=[2,7,3.5,3.5])

w6 >> rave([0,8,12,17],dur=PDur(3,4))
